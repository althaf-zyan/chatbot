...
FALLBACK_REPLY = "Sorry, I could not process that right now. Please try again soon."
...
def safe_sender_label(sender: str | None) -> str:
    if not sender:
        return "unknown"
    return f"ending in {sender[-4:]}"
...
def process_text_message(sender: str, message_body: str) -> None:
    print(f"Processing text message from sender {safe_sender_label(sender)}")
    ai_reply = get_groq_reply(message_body) or FALLBACK_REPLY
    sent = send_whatsapp_text(sender, ai_reply)
    if sent:
        print(f"Reply sent to sender {safe_sender_label(sender)}")
    else:
        print(f"Reply failed for sender {safe_sender_label(sender)}")
...
def handle_webhook():
    try:
        ...
        if not message_body:
            print(f"Ignoring empty text message from sender {safe_sender_label(sender)}")
            return jsonify({"status": "ignored"}), 200

        print(f"Accepted text message from sender {safe_sender_label(sender)}")
        thread = threading.Thread(
            target=process_text_message,
            args=(sender, message_body),
            daemon=True,
        )
        thread.start()
